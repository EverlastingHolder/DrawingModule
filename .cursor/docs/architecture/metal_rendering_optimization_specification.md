...
### 2.2 Smudge Engine Optimization
*   **Threadgroup Memory**: Использование 2KB локальной памяти `threadgroup` для кеширования пикселей холста, что снижает нагрузку на VRAM.
*   **Half-Precision (половинная точность)**: Все вычисления в `compute_smudge_hdr` производятся с точностью `half` для экономии регистров и повышения пропускной способности.
*   **Register Pressure**: Поддержание уровня использования регистров <= 32 на поток для достижения 120 FPS.
...
## 5. Динамическое управление качеством и архитектура

### 5.1 Dynamic Quality Scaling
Для обеспечения стабильного фреймрейта внедрен механизм адаптивного изменения качества с гистерезисом:
*   **Downscale (Упрощение)**: Активируется при времени кадра > 9.0мс.
*   **Upscale (Восстановление)**: Возврат к полному качеству только при времени кадра < 7.5мс.
Это предотвращает мерцание качества (jitter) при пограничных нагрузках.

### 5.2 Кроссплатформенность: Стратегия Fallback
...
*   Система определяет IMR архитектуру через `supportsFamily(.apple1)`. Если false — используется `isIMRArchitecture = true`.
